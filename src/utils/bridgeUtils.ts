// Shared bridge geometry — imported by both FarmMap.tsx and Player.tsx
// so the arch curve used for rendering matches the one used for player Y.

export interface BridgeGap { zMin: number; zMax: number; }

export const BRIDGE_CX: number[]   = [-60, 0, 60];
export const BRIDGE_HALF: number = 6;
export const ARCH_PEAK: number   = 2;   // keep in sync with FarmMap constant
export const GAPS: BridgeGap[] = [
  { zMin: -44, zMax: -33 },
  { zMin:  33, zMax:  44 },
];
...
export function getBridgeY(x: number, z: number): number {
