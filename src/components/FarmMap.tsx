// FarmMap.tsx — Voxel islands connected by wooden arch bridges

import React, { useRef, useEffect, useMemo, useState } from "react";
import * as THREE from "three";
import { Text } from "@react-three/drei";
import { BRIDGE_CX, BRIDGE_HALF, ARCH_PEAK, GAPS } from "../utils/bridgeUtils";
import type { BridgeGap } from "../utils/bridgeUtils";
...
// ── Island z-extents ──────────────────────────────────────────────────────
const ISLANDS: Record<string, { zMin: number; zMax: number }> = {
...
// ── Zone layout ───────────────────────────────────────────────────────────
const COL_BOUNDS: [number, number][] = [
...
function colIdx(x: number): number {
...
function islandId(z: number): string | null {
...
function createGroundTexture(type: string): THREE.CanvasTexture {
  const SZ = 256;
  ...
  const cfgs: Record<string, ZoneCfg> = {
...
const GROUND_TEX: Record<string, THREE.CanvasTexture> = Object.fromEntries(ZONE_TYPES.map(t => [t, createGroundTexture(t)]));

interface BlockGroupData { typeName: string; positions: { x: number; z: number }[] }

// ── Build island block groups (keyed by zone type for texture sharing) ────
function buildGroups(): BlockGroupData[] {
  const map: Record<string, BlockGroupData> = {};
...
interface BlockGroupProps { typeName: string; positions: { x: number; z: number }[]; height?: number; }

// ── Textured block group ──────────────────────────────────────────────────
function BlockGroup({ typeName, positions, height = PH }: BlockGroupProps): JSX.Element {
  ...
  return (
    <instancedMesh ref={ref} args={[undefined, undefined, positions.length]} receiveShadow castShadow>
...
interface RockEntry { x: number; z: number; sc: number; sy: number; ry: number; }

// ── Scattered rocks ───────────────────────────────────────────────────────
function ScatteredRocks(): JSX.Element {
  const { light, dark } = useMemo(() => {
    const light: RockEntry[] = [], dark: RockEntry[] = [];
    for (let xi = 0; xi < 60; xi++) {
      for (let zi = 0; zi < 78; zi++) {
        const x = -90 + xi * BS + BS * 0.5;
        const z = -117 + zi * BS + BS * 0.5;
        ...
        const entry: RockEntry = { x: x + ox, z: z + oz, sc, sy, ry };
  ...
  useEffect(() => {
    const dummy = new THREE.Object3D();
    const pairs: [React.RefObject<THREE.InstancedMesh | null>, RockEntry[]][] = [[lRef, light], [dRef, dark]];
    pairs.forEach(([ref, arr]) => {
  ...
  return (
    <group>
      <instancedMesh ref={lRef} args={[undefined, undefined, light.length]} castShadow receiveShadow>
        <icosahedronGeometry args={[1, 1]} />
        <meshStandardMaterial color="#828070" roughness={0.96} />
      </instancedMesh>
      <instancedMesh ref={dRef} args={[undefined, undefined, dark.length]} castShadow receiveShadow>
        <icosahedronGeometry args={[1, 1]} />
...
interface TuftEntry { x: number; z: number; h: number; ry: number; dark: boolean; }

// ── Grass tufts ───────────────────────────────────────────────────────────
const GREEN_ZONES = new Set(['plains', 'village', 'training', 'forest']);

function GrassTufts(): JSX.Element {
  const tufts = useMemo(() => {
    const arr: TuftEntry[] = [];
    for (let xi = 0; xi < 60; xi++) {
      for (let zi = 0; zi < 78; zi++) {
  ...
  useEffect(() => {
    const dummy = new THREE.Object3D();
    const pairs: [React.RefObject<THREE.InstancedMesh | null>, number][] = [[refA, 0], [refB, Math.PI * 0.5]];
    pairs.forEach(([ref, extra]) => {
      if (!ref.current) return;
  ...
  return (
    <group>
      <instancedMesh ref={refA} args={[undefined, undefined, tufts.length]}>
        <planeGeometry args={[1, 1]} />
        <meshStandardMaterial color="#2e6a10" side={THREE.DoubleSide} roughness={0.9} />
      </instancedMesh>
      <instancedMesh ref={refB} args={[undefined, undefined, tufts.length]}>
        <planeGeometry args={[1, 1]} />
...
interface SingleBridgeProps { bx: number; gap: BridgeGap; }

// ── Single wooden arch bridge ─────────────────────────────────────────────
function SingleBridge({ bx, gap }: SingleBridgeProps): JSX.Element {
  const { zMin, zMax } = gap;
  ...
  const { railGeoL, railGeoR, segments } = useMemo(() => {
    const segments: { z: number; y: number; tilt: number }[] = [];
    const railPtsL: THREE.Vector3[] = [];
    const railPtsR: THREE.Vector3[] = [];
...
// ── Arch bridges ──────────────────────────────────────────────────────────
function ArchBridges(): JSX.Element {
  return (
    <group>
      {GAPS.flatMap((gap, gi) =>
        BRIDGE_CX.map((bx, bi) => (
...
interface FarmMapProps { onLoaded?: () => void; onShopClick?: () => void; playerCount?: number; }

// ── Main map ──────────────────────────────────────────────────────────────
export function FarmMap({ onLoaded, onShopClick }: FarmMapProps): JSX.Element {
...
interface MapFenceProps { width: number; length: number; }
interface RailConfig { pos: [number, number, number]; rot: [number, number, number]; len: number; }

// ── Perimeter fence ───────────────────────────────────────────────────────
function MapFence({ width, length }: MapFenceProps): JSX.Element {
  ...
  const mats = useMemo(() => {
    const arr: THREE.Matrix4[] = [], dummy = new THREE.Object3D();
    const line = (sx: number, sz: number, dx: number, dz: number, n: number, a: number) => {
      for (let i = 0; i < n; i++) {
        const t = i / n;
  ...
  return (
    <group>
      <instancedMesh ref={pRef} args={[undefined, undefined, total]} castShadow>
        <boxGeometry args={[0.3, pH, 0.08]} />
        <meshStandardMaterial color="#1a1a1a" roughness={0.8} />
      </instancedMesh>
      <instancedMesh ref={tRef} args={[undefined, undefined, total]} castShadow>
...
interface CenterBarnProps { onShopClick?: () => void; }

// ── Shop barn ─────────────────────────────────────────────────────────────
function CenterBarn({ onShopClick }: CenterBarnProps): JSX.Element {
  const [hovered, setHovered] = useState(false);
  const interact = {
    onClick:       (e: THREE.Event) => { (e as unknown as MouseEvent).stopPropagation?.(); if (onShopClick) onShopClick(); },
      ...
      {sides.map((s, i) => (
        <group key={i}>
          <mesh position={[s.pos[0], 1.2+s.pos[1], s.pos[2]]}>
            <boxGeometry args={s.da as [number, number, number]} /><meshStandardMaterial color="#3E2723" roughness={0.9} />
          </mesh>
          <mesh position={[s.cp[0], 1.2+s.cp[1], s.cp[2]]}>
            <boxGeometry args={s.cv as [number, number, number]} /><meshStandardMaterial color="#5D4037" />
          </mesh>
          <mesh position={[s.cp[0], 1.2+s.cp[1], s.cp[2]]}>
            <boxGeometry args={s.ch as [number, number, number]} /><meshStandardMaterial color="#5D4037" />
          </mesh>
          <group position={s.sp as [number, number, number]} rotation={s.sr as [number, number, number]}>
