// soundFx.js — Lightweight Web Audio sound effects.
...
function tone({ freq = 440, duration = 0.2, type = "sine", volume = 0.2, attack = 0.005, release = 0.12, when = 0 }) {
  if (!_enabled) return;
  const ac = ctx();
  if (!ac) return;
...
function sweep({ from = 200, to = 600, duration = 0.25, type = "triangle", volume = 0.25, when = 0 }) {
  if (!_enabled) return;
  const ac = ctx();
  if (!ac) return;
  const t0 = ac.currentTime + when;
