// DogHouse.tsx — Dog House with white picket fence + dog
...
interface FoodBowlProps { position: [number, number, number]; }

export function DogHouse({ position, rotation = [0, 0, 0], player, isEating, onInventoryClick }: DogHouseProps): JSX.Element {
...
function PicketFence(): JSX.Element {
  const fenceWidth = 9.5;
  const fenceDepth = 9.5;
  const picketHeight = 1.6;
  const picketWidth = 0.3;

  function FenceSegment({ start, end, isGate = false }: FenceSegmentProps): JSX.Element {
    ...
    // tStart..tEnd along this fence segment.
    function rail(tStart: number, tEnd: number, y: number, key: string): JSX.Element {
...
// ── Food bowl ─────────────────────────────────────────────────────────────
function FoodBowl({ position }: FoodBowlProps): JSX.Element {
  return (
